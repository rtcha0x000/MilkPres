...
MRichTextEdit::MRichTextEdit(QWidget *parent) : QWidget(parent) {
    ...
    connect(this->f_font, SIGNAL(currentFontChanged(QFont)),
            this, SLOT(textFont(QFont)));
...
void MRichTextEdit::textFont(QFont font){
    QTextCharFormat fmt;
    fmt.setFont(font);
    mergeFormatOnWordOrSelection(fmt);
}
...
{
    emit sendText(f_textedit->toHtml());
    close();
}
